...
template<class T,size_t S>
std::ostream &      operator<<(std::ostream & os,const Arr<T,S> & arr)
{
    os << "[";
    for (T const & e : arr)
        os << e << " ";
...
Arr<T,S>            mapCall(Arr<T,S> const & lhs,Arr<T,S> const & rhs,F func)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
...
// * we can't use mapCall with std::plus<T> etc. since T may not be in std::
// * we can't use mapCall with operator+<T> etc since it's potentially ambiguous
template<class T,size_t S>
Arr<T,S>             operator+(Arr<T,S> const & l,Arr<T,S> const & r)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = l[ii] + r[ii];
...
template<class T,size_t S>
void                operator+=(Arr<T,S> & l,Arr<T,S> const & r)
{
    for (size_t ii=0; ii<S; ++ii)
        l[ii] += r[ii];
}
...
template<class T,size_t S>
Arr<T,S>            mapAdd(Arr<T,S> const & lhs,T rhs)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = lhs[ii] + rhs;
...
template<class T,size_t S>
Arr<T,S>            mapSub(Arr<T,S> const & lhs,T rhs)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = lhs[ii] - rhs;
...
template<class T,size_t S>
Arr<T,S>            operator*(Arr<T,S> const & l,T r)
{
    Arr<T,S>      ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = l[ii] * r;
...
// * can't use mapCall for same reason as operator+/-
template<typename T,size_t S>
Arr<T,S>            mapMul(Arr<T,S> const & l,Arr<T,S> const & r)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = l[ii] * r[ii];
...
template<typename T,size_t S>
Arr<T,S>            mapDiv(Arr<T,S> const & l,Arr<T,S> const & r)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = l[ii] / r[ii];
...
template<class T,size_t S>
Arr<T,S>            mapExp(Arr<T,S> const & a)
{
    Arr<T,S>                ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = std::exp(a[ii]);
...
Arr<To,S>           mapCast(Arr<From,S> const & v)
{
    Arr<To,S>    ret;
    for (size_t ii=0; ii<S; ++ii)
...
template<typename To,typename From,size_t S>
void                deepCast_(Arr<From,S> const & from,Arr<To,S> & to)
{
    for (size_t ii=0; ii<S; ++ii)
        deepCast_(from[ii],to[ii]);
}
template<class T,size_t S>
Arr<T,S>            mapSqr(Arr<T,S> const & a)
{
    Arr<T,S>        ret;
    for (size_t ii=0; ii<S; ++ii)
        ret[ii] = a[ii] * a[ii];
...
template<class T,size_t S>
Arr<T,S>            normalize(Arr<T,S> const & a)
{
    T               acc {0};
    for (size_t ii=0; ii<S; ++ii)
        acc += a[ii] * a[ii];
