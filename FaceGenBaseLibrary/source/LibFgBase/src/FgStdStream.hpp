...
struct  Ofstream : public std::ofstream
{
    ...
    template<class T>
    void            writeb(T const & val) {write(reinterpret_cast<char const*>(&val),sizeof(val)); }
    // raw binary output from string:
    void            writeBytes(String const & s)
    {
        if (!s.empty())
            write(s.data(),s.size());
    }
...
template<class T>
void                fgWriteb(std::ostream & os,T const & val)    // Only use for builtins !
...
inline void         fgWritep(std::ostream & os,String const & str)
{
...
inline void         fgWritep(std::ostream & os,String8 const & s) {fgWritep(os,s.m_str); }

template<class T>
void                fgWritep(std::ostream & os,Svec<T> const & vec)
{
...
struct  Ifstream : public std::ifstream
{
    ...
    template<typename T>
    void            readb_(T & val) {read(reinterpret_cast<char*>(&val),sizeof(val)); }
    template<typename T>
    T               readb()
    {
        T       ret;
...
template<class T>
void                readb(std::istream & is,T & val)
{
    is.read(reinterpret_cast<char*>(&val),sizeof(T));
}

template<class T>
T                   fgReadt(std::istream & is)
{
    T       ret;
...
inline void         fgReadp(std::istream & is,String & str)
{
...
inline void         fgReadp(std::istream & is,String8 & str) {fgReadp(is,str.m_str); }

template<class T>
void                fgReadp(std::istream & is,Svec<T> & vec)
{
